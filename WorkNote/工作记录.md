...
Dag图(Service map)
- 完成 cilium-exporter 开发，实现hubble flow数据的采集，提供自定义标签与简单的条件过滤，为dag图聚合提供原始数据

agent
- 优化agent构建过程，加速agent的部署与监控组件的启动
  - 缩小36%可执行文件大小，减小50% agent镜像大小，加速agent部署过程

算力度量对接
- 完成 perf-exporter 开发，将应用与 perf-exporter 一同容器化，配置响应执行脚本，实现应用的可迁移运行，实现算力度量数据的采集和展示
  - 监听 `perf stats` 命令输出(文件), 将原始数据转化为指标信息，发送至pushgateway, 并通过聚合规则在prometheus界面窗口进行查询展示，打通监控流程
- 将应用与 perf-exporter 一同容器化，配置响应执行脚本，实现应用的可迁移运行 