...
当从trap_handler中返回时, a0为先前传入的TrapContext指针，也即内核栈指针，故进入`__restore`执行`mv sp, a0`时，sp不变(恢复先前的内核栈指针)
当从`run_next_app`调用时，显然也是内核指针，且指向一个内核为app创建的TrapContext，因而`__restore`的行为表现为初通过TrapContext初始化应用程序的执行环境(包括用户栈)，并通过`sret`切换为用户态并指向用户app的代码开始执行
...
__restore:
    ...
    ld t1, 33*8(sp)
    ld t2, 2*8(sp) # 通过读取TrapContext内容，恢复用户栈指针
    csrw sstatus, t0
    csrw sepc, t1
    csrw sscratch, t2
    ld x1, 1*8(sp)
...
`push_context`的返回值是内核栈压入Trap上下文之后的栈顶，它会被作为`__restore`的参数，这使得在`__restore`函数中`sp`仍然可以指向内核栈的栈顶
